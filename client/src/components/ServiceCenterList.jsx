    ...
    const filteredCenters = useMemo(
      () =>
        centers.filter((center) => {
          ...
          return (
            center.objectData.name?.toLowerCase().includes(norm) ||
            center.objectData.address?.toLowerCase().includes(norm) ||
            center.objectData.services?.toLowerCase().includes(norm)
          );
        }),
      [centers, searchTerm]
    );
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        {loading ? (
          ...
          <div className="text-center py-12 bg-white rounded-xl">
            <div className="icon-map-pin text-5xl text-gray-300 mb-4" />
                  ...
                  {[1, 2, 3, 4, 5].map((star) => (
                    <div
                      ...
                      className={`icon-star text-lg ${
                        star <= (center.objectData.rating || 0) ? 'text-yellow-500' : 'text-gray-300'
                      }`}
                    />
                  ))}
                  <span className="ml-2 text-sm text-gray-600">({center.objectData.reviewCount || 0})</span>
                </div>
                <p className="text-sm text-gray-500">{center.objectData.services}</p>
              </div>
            ))}
          </div>
        )}
