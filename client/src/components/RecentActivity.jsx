...
export default function RecentActivity() {
  try {
    ...
    return (
      <div className="bg-white rounded-xl p-6 shadow" data-name="recent-activity">
        <h3 className="text-xl font-bold mb-4">Recent Activity</h3>
        {loading ? (
          <p className="text-center py-8 text-gray-500">Loading...</p>
        ) : activities.length === 0 ? (
          <div className="text-center py-8">
            <div className="icon-activity text-4xl text-gray-300 mb-2" />
            ...
            {activities.map((activity) => (
              <div key={activity.objectId} className="flex items-start space-x-3 pb-4 border-b last:border-0">
                <div className="w-10 h-10 bg-green-100 rounded-lg flex items-center justify-center flex-shrink-0">
                  <div className="icon-check text-lg text-green-600" />
                </div>
                <div className="flex-1">
                  <p className="font-semibold">
                    {activity.objectData.ServiceType || activity.objectData.serviceType || 'Maintenance'}
                  </p>
