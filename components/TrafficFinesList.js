function TrafficFinesList() {
  try {
    ...
    const totalPending = fines.filter(f => f.objectData.Status === 'Pending')
      .reduce((sum, f) => sum + (f.objectData.Amount || 0), 0);
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-8">
          ...
          <div className="bg-white rounded-xl shadow p-6">
            ...
            <p className="text-3xl font-bold text-green-600">
              {fines.filter(f => f.objectData.Status === 'Paid').length}
            </p>
          </div>
        </div>
        ...
        {fines.length === 0 ? (
          <div className="bg-white rounded-xl p-12 text-center">
            <div className="icon-shield-check text-5xl text-green-500 mb-4"></div>
          ...
          <div className="space-y-4">
            {fines.map((fine, index) => {
              ...
              return (
                <div key={fine.objectId} className={`bg-white rounded-xl shadow hover:shadow-lg p-6 border-l-4 transition-all duration-300 ${isPending ? (isOverdue ? 'border-red-500' : 'border-orange-500') : 'border-green-500'}`} style={{animation: 'slideUp 0.5s ease-out', animationDelay: `${index * 0.05}s`, animationFillMode: 'backwards'}}>
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center space-x-3 mb-2">
                        <h3 className="text-xl font-bold">{fine.objectData.FineType}</h3>
                        <span className={`px-3 py-1 rounded-full text-sm font-semibold ${
                          fine.objectData.Status === 'Paid' ? 'bg-green-100 text-green-800' : 
                          fine.objectData.Status === 'Appealed' ? 'bg-blue-100 text-blue-800' : 
                      ...
                      <div className="grid md:grid-cols-2 gap-4 text-sm">
                        <p><span className="font-semibold">Location:</span> {fine.objectData.Location}</p>
                        <p><span className="font-semibold">Date:</span> {new Date(fine.objectData.Date).toLocaleDateString()}</p>
                        <p><span className="font-semibold">Due:</span> {dueDate.toLocaleDateString()}</p>
                        {fine.objectData.PaymentReference && (
                          <p><span className="font-semibold">Ref:</span> {fine.objectData.PaymentReference}</p>
                        )}
                      </div>
                    </div>
                    <div className="text-right">
                      ...
                      {isPending && (
                        <button onClick={() => handlePayment(fine)} className="px-6 py-2 bg-[var(--primary-color)] text-white rounded-lg font-semibold hover:opacity-90">
                          Pay with Fawry
                        </button>
                      )}
                    </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-xl max-w-md w-full p-6">
              ...
              <div className="flex space-x-3">
                <button onClick={processPayment} className="flex-1 bg-[var(--primary-color)] text-white py-3 rounded-lg font-semibold hover:opacity-90">
                  Mark as Paid
                </button>
                <button onClick={() => setShowPayModal(false)} className="flex-1 border-2 border-gray-300 py-3 rounded-lg font-semibold hover:bg-gray-50">
                  Cancel
                </button>
              </div>
            </div>
